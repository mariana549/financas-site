...
function _noteRunAI(text, noteId) {
  // Reutiliza o parser de IA já existente
  if (typeof openAIImport === 'function') {
    openAIImport(text, noteId);
  } else {
    // Abre o modal de importação pré-preenchido
    showView('dash');
    setTimeout(() => {
      const aiBtn = document.querySelector('[onclick*="openAIImport"]');
      if (aiBtn) aiBtn.click();
      const aiInput = document.getElementById('aiText');
      if (aiInput) { aiInput.value = text; }
    }, 200);
    showToast('Cole o texto no campo de IA e processe.');
  }
}
