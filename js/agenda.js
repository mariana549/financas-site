...
function _renderAgendaMonth(el, monthKey) {
  ...
  const rows = filtered.map(item => {
    ...
    // Todos os itens são clicáveis para marcar como pago
    const safeBank = (item.bankName || '').replace(/'/g, "\\'");
    const toggleAttr = `onclick="toggleAgendaItemPaid('${item.type}','${item.id}','${monthKey}','${safeBank}')"
      title="Clique para marcar como ${item.paid ? 'pendente' : 'pago'}" style="cursor:pointer"`;
    ...
    ${filtered.length === 0
      ...
      : `<div style="padding:0 20px 60px">
          ...
          <div style="font-size:11px;color:var(--text3);margin-top:10px;line-height:1.5">
            💡 Clique no badge de status para marcar qualquer item como pago/pendente.
          </div>
        </div>`}`;
}
