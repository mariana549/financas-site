// ══════════════════════════════════════════════════
// DASHBOARD.JS — Visão Geral do Mês
// ══════════════════════════════════════════════════
...
function renderDash() {
  ...
  const goalBar = m.goal ? `
    <div class="goal-bar-wrap" style="margin-top:6px">
      <div class="goal-bar-track">
        <div class="goal-bar-fill" style="width:${Math.min(totalGasto / m.goal * 100, 100).toFixed(1)}%;background:${
          totalGasto > m.goal ? 'var(--red)' : totalGasto / m.goal > 0.8 ? 'var(--orange)' : 'var(--green)'
        }"></div>
      </div>
  ...
  // ── Alertas ──
  let alerts = '';
  if (m.goal && totalGasto / m.goal > 0.8 && totalGasto <= m.goal) {
  ...
  if (m.goal && totalGasto > m.goal) {
    alerts += `<div class="alert-banner" style="background:#ff4d4d18;border-color:#ff4d4d44;color:var(--red)">🚨 Meta de gastos ultrapassada em R$ ${fmt(totalGasto - m.goal)}.</div>`;
  }
  ...
  recL.forEach(r => {
    if (r.day) {
      ...
      if (diff >= 0 && diff <= 3) {
        ...
        alerts += `<div class="alert-banner">📅 ${r.desc} vence ${when} (dia ${r.day}).</div>`;
      }
    }
  });
  ...
  const bkTabs = m.banks.map(b => `
    <div class="btab ${S.currentBank === b.name ? 'active' : ''}" onclick="selectBank('${b.name}')">
      <span class="dot" style="color:${PALETTE[b.color] || PALETTE.azure}"></span>${b.name}
    </div>`).join('') +
    `<button class="btab" onclick="openModal('mBank')" style="opacity:.5">+</button>`;
  ...
  if (!m.banks.length && !recL.length && !pixL.length) {
    ...
    if (m.banks.length) {
      gastoHTML += `
        <div class="sec-title" style="margin-bottom:8px">Bancos</div>
        <div class="bank-tabs">${bkTabs}</div>`;
      ...
      if (bk) {
        ...
        const rows = sorted.length ? sorted.map(e => {
          ...
          return `
            <tr class="entry-row" onclick='showEntryDetail(${safeEntry},"${bk.name}")'>
              <td>${e.desc} ${ib}</td>
              <td>${wb}</td>
    ...
    if (pixL.length) {
      ...
      gastoHTML += `
        <div class="tbl-block">
          <div class="tbl-head">
            <span class="tbl-title">Pix Enviados</span>
            <span class="tbl-total" style="color:var(--green)">R$ ${fmt(pixT)}</span>
          </div>
          <table><thead><tr><th>Para</th><th>Tipo</th><th>Valor</th></tr></thead>
    ...
    if (recL.length) {
      ...
      gastoHTML += `
        <div class="tbl-block">
          <div class="tbl-head">
            <span class="tbl-title">Contas Fixas</span>
            <span class="tbl-total" style="color:var(--orange)">R$ ${fmt(recT)}</span>
          </div>
          <table><thead><tr><th>Descrição</th><th>Tipo</th><th>Valor</th></tr></thead>
  ...
  if (!incL.length) {
    ...
    const rows = [...incL].sort((a, b_) => new Date(b_.date) - new Date(a.date)).map(i => `
      <tr class="entry-row" onclick="openIncomeM(${i.id})">
        ...
        <td>${i.owner === 'other'
          ? `<span class="bm bm-other">${i.person}</span>`
          : `<span class="bm bm-mine">meu</span>`}</td>
    ...
    entradaHTML += `
      ...
      <div class="tbl-block">
        ...
        <table><thead><tr><th>Descrição</th><th>De quem</th><th>Valor</th></tr></thead>
  ...
  el.innerHTML = `
    ...
    <div class="summary-grid">
      ...
      <div class="card">
        ...
        <div class="card-val a">R$ ${fmt(myT + pixT + recT)}</div>
      </div>
      <div class="card">
        <div class="card-lbl">A Receber</div>
        <div class="card-val b">R$ ${fmt(othT)}</div>
        <div class="card-sub">${Object.keys(pplMap).length} pessoa(s)</div>
      </div>
      <div class="card">
      ...
      <div class="card">
        ...
        <div class="card-val ${saldo >= 0 ? 'g' : 'r'}">R$ ${fmt(saldo)}</div>
      </div>
      ${subM > 0 ? `
      <div class="card">
    ...
    <div class="itab-content" id="itabc-entradas" style="display:none">
      ${entradaHTML}
    </div>`;
